 ...
 *      prevArrow - Image - Pass in an image to replace default image. Default: (new Image()).src="./images/prev.png"
 *      lastArrow - Image - Pass in an image to replace default image. Default: (new Image()).src="./images/last.jpg"
...
(function(a){
    a.fn.tablePagination=function(b){
        ...
        return this.each(function(){
            ...
            function p(){
                ...
                for(var w=0;w<z.length;w++){
                    if(z[w]==c.rowsPerPage){
                        x.options[w]=new Option(z[w],z[w],true,true);
                        y=true
                    }else{
            ...
            function g(){
                var w=[];
                w.push("<div id='tablePagination'>");
                w.push("<span id='tablePagination_perPage'>");
                w.push("<select id='tablePagination_rowsPerPage'><option value='5'>5</option></select>");
                w.push("per page");
                w.push("</span>");
                w.push("<span id='tablePagination_paginater'>");
                w.push("<img id='tablePagination_firstPage' src='"+c.firstArrow+"'>");
                w.push("<img id='tablePagination_prevPage' src='"+c.prevArrow+"'>");
                w.push("Page");
                w.push("<input id='tablePagination_currPage' type='input' value='"+m+"' size='1'>");
                w.push("of <span id='tablePagination_totalPages'>"+v+"</span>");
                w.push("<img id='tablePagination_nextPage' src='"+c.nextArrow+"'>");
                w.push("<img id='tablePagination_lastPage' src='"+c.lastArrow+"'>");
                w.push("</span>");
                w.push("</div>");
            ...
            p();
            o(m);
            a(s)[r]().find(i).bind("click",function(w){
                n(1)
            });
            a(s)[r]().find(j).bind("click",function(w){
                n(m-1)
            });
            a(s)[r]().find(u).bind("click",function(w){
                n(parseInt(m)+1)
            });
            a(s)[r]().find(q).bind("click",function(w){
                n(v)
            });
            a(s)[r]().find(f).bind("change",function(w){
                n(this.value)
            });
            a(s)[r]().find(l).bind("change",function(w){
                c.rowsPerPage=parseInt(this.value,10);
                v=e();
                n(1)
            })
        })
